...
for iitt=1:20
 %% algorithm
 %rng('default');
 Nng = randi(K, N,1);  % initialize: each UAV in group g accesses to a BS randomly
 %Nng(1:10000) = 3;
 Bn = [B/3;B/3;B/3];
 Bn_new = zeros(K,1);
 Bn_iter = [];
...
Nng_2 = Nng;
 for it=1:iter
     ...
     xng = [a1/N,a2/N,a3/N];
     xng_iter = [xng_iter;[a1/N,a2/N,a3/N]];
...
haha = [haha;xng_iter];  % run iitt times to test the robustness of the result
avepay = [avepay;mean(Payoff_UAV)];
end
 %% figure

figure(2); grid on
hold on
plot(xng_iter(:,1),'-go','Linewidth',2);hold on;
plot(xng_iter(:,2),'-b*','Linewidth',2);hold on;
plot(xng_iter(:,3),'-ms','Linewidth',2)
...
legend('#MDs connect to UAV1','#MDs connect to UAV2','#MDs connect to UAV3');
 ...
 figure(3);grid on
 plot(payoff_ave_whole,'-o','Linewidth',2);
